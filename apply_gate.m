function gate_out = update_gate(channels_to_gate, channels_to_scale, load_gate)
...
% out, generally...
% channels_to_gate = {'FSC-A', 'YL2-A','Include'}; % Specify pairs of channels for gating
% channels_to_scale = {'log','log'}; % Specify scale for each pair of channels
...
for n_gate = 1:size(channels_to_gate,1)
    ...
    % Incorporate loaded gate
    lastGate = size(load_gate,1);
    idxL = load_gate{lastGate,4};
        if strcmp(load_gate{lastGate,6},'Subtract')
            xdata = xdata(~idxL);
            ydata = ydata(~idxL);
        else
            xdata = xdata(idxL);
